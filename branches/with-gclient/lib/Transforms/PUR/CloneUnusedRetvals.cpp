...
namespace {
  ...
  struct CloneUnusedRetvals :
    ...
    CloneUnusedRetvals() : ModulePass(ID) {
      initializeCloneUnusedRetvalsPass(*PassRegistry::getPassRegistry());
    }
...
INITIALIZE_PASS(CloneUnusedRetvals, "clone-unused-retvals", "Clone unused retvals functions", false, false)

ModulePass *llvm::createCloneUnusedRetvalsPass() {
  return new CloneUnusedRetvals();
}
