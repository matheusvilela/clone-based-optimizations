...
namespace {
  ...
  struct PruneClones :
    ...
    PruneClones() : ModulePass(ID), changed(false) {
      initializePruneClonesPass(*PassRegistry::getPassRegistry());
    }
...
INITIALIZE_PASS(PruneClones, "prune-clones", "Prune worthless function clones", false, false)

ModulePass *llvm::createPruneClonesPass() {
  return new PruneClones();
}
