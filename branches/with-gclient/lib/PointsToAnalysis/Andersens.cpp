...
namespace {
  struct BitmapKeyInfo : public DenseMapInfo<SparseBitVector<> *> {
...
#define DEBUG_TYPE "anders-aa"
  ...
  private:
    ...
    void visitCallInst(CallInst &CI) { 
      if (isMallocLikeFn(&CI, TLI)) visitAlloc(CI);
...
char Andersens::ID = 0;
INITIALIZE_AG_PASS(Andersens, AliasAnalysis, "anders-aa",
  "Andersen's Interprocedural Alias Analysis", false, true, false);
...
AliasAnalysis::AliasResult Andersens::alias(const Location &LocA, const Location &LocB) {
  Node *N1 = &GraphNodes[FindNode(getNode(const_cast<Value*>(LocA.Ptr)))];
  Node *N2 = &GraphNodes[FindNode(getNode(const_cast<Value*>(LocB.Ptr)))];
...
void Andersens::PrintNode(const Node *N) const {
  ...
  if (isa<GlobalValue>(V) || isa<AllocaInst>(V) || isMallocLikeFn(V, TLI))
